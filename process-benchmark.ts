...
const valkeyBenchmarkOutput = await runBenchmark("docker run --rm --network host redis:7.4.2-bookworm redis-benchmark -3 -n 1000 -c 500 -P 10 -q -p 6379 --csv")
valkeyServer.kill()
...
const kvrocksBenchmarkOutput = await runBenchmark("docker run --rm --network host redis:7.4.2-bookworm redis-benchmark -3 -n 1000 -c 500 -P 10 -q -p 6666 --csv")
kvrocksServer.kill()
...
writeFileSync("results.md", md.replace(/(?<=<!-- chart-begin -->\n)[\S\s]*(?=\n<!-- chart-end -->)/, mermaid))
